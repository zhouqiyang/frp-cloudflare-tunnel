...
echo "Service started at $(date -d "$(awk -F. '{print $1}' /proc/uptime) second ago" +"%Y-%m-%d %H:%M:%S") and has been running for $(cat /proc/uptime| awk -F. '{run_days=$1 / 86400;run_hour=($1 % 86400)/3600;run_minute=($1 % 3600)/60;run_second=$1 % 60;printf("%d day(s) %02d:%02d:%02d\n",run_days,run_hour,run_minute,run_second)}')" >> $LOG_FILE
cat $LOG_FILE
